...
t[0]==e[0]&&(i+=1);for(var r=0;r<n;r++){var u=t.indexOf(e.charAt(r).toLowerCase());switch(u){case-1:return 0;// not found, not our word
case 0:i+=.6,// first position, good
r==s&&(// consecutive, better
i+=.4);break;default:i+=.4/s}s+=u,t=t.substr(u+1)}
...
setLength:function(t){this.length=t},/**
	 ...
	 */
_updateScroll:function(){var t=this.current-this.scrolled;t>4?this.scrolled+=t-4:t<=0&&(this.scrolled+=t),this.scrollWrap.scrollTop(this.scrolled*this.active.outerHeight())},/**
 ...
 */
var s={enter:13,escape:27,up:38,right:39,down:40};t.fn.fuzzyFilter=function(){return this.each(function(){if(!t(this).data("fuzzyFilterActive")){t(this).data("fuzzyFilterActive",!0);var n=t(this),r=t(this).closest(".sub-menu").find("ul"),u=r.closest(".scroll-wrap"),h=new i(r),c=new e(r,{keep:".last:has(.add)"});
...
n.on("focus",function(){u.width(u.width())}),n.on("keydown",function(t){switch(h.setLength(c.length),t.keyCode){case s.enter:t.preventDefault(),h.getCurrent()[0].click();break;case s.escape:n.val("");break;case s.up:h.up();break;case s.down:h.down();break;default:return}t.preventDefault()}),n.on("interact",function(){c.filter(n.val()),h.setCurrent(0)})}})},t.fuzzyFilter=function(){t(".filter input, .filters input[data-fuzzy-filter=true]").fuzzyFilter()},
